<?xml version="1.0" encoding="UTF-8"?>
<schema attributeFormDefault="unqualified" elementFormDefault="qualified" targetNamespace="<%= spec.csar_namespace %>" xmlns:tns="<%= spec.csar_namespace %>" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/2001/XMLSchema">

  <!-- <element name="ArtifactProperties">
    <complexType>
      <sequence>
        <any minOccurs="0" maxOccurs="unbounded" namespace="##targetNamespace"/>
      </sequence>
    </complexType>
  </element> -->

  <% _.forEach(spec.node_types, function(nt, ntName) { %>
  <% if (!_.isEmpty(nt.properties_schema)) { %>
  <element name="<%= ntName %>_Properties">
    <complexType>
      <sequence>
        <% _.forEach(nt.properties_schema, function(p, pName) { %>
        <element name="<%= pName %>" type="<%= p.type %>" <% if (p.default && _.isString(p.default)) { %>default="<%= p.default %>"<% } %> minOccurs="0" maxOccurs="1"/>
        <% }); %>
        <any minOccurs="0" maxOccurs="unbounded" namespace="##targetNamespace"/>
      </sequence>
    </complexType>
  </element>
  <% } %>
  <% }); %>

  <% if (spec.xsd_types_xml) { %>
  <%= spec.xsd_types_xml %>
  <% } %>

</schema>
