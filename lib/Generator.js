...
//TODO distinguish between files (not prefixed with workingDir or outputDir) and resolvedFiles; use files for IGNORE and TOSCA.meta!
        glob(path.join(context.workingDir, '**'), {
          ...
          glob(path.join(context.outputDir, '**'), {
            //mark: true, // add '/' character to directory matches
            nodir: true,
            dot: true
          }, (err, outputDirFiles) => {
            ...
            let files = workingDirFiles.concat(outputDirFiles);
            ...
            csarSpec.files = _.uniq(_.map(csarSpec.files.concat(files), (filePath) => {
        ...
        try {
          ...
          _.forEach(csarSpec.artifacts, (a, aName) => {
            _.forEach(a.references, (ref) => {
              if (_.isString(ref) &&
                 !_.includes(csarSpec.files, path.resolve(context.workingDir, ref)) &&
                 !_.includes(csarSpec.files, path.resolve(context.outputDir, ref))) {
            ...
            if (refProperties[a.type]) {
              _.forEach(refProperties[a.type], (propName) => {
                if (_.isString(a[propName]) &&
                   !_.includes(csarSpec.files, path.resolve(context.workingDir, a[propName])) &&
                   !_.includes(csarSpec.files, path.resolve(context.outputDir, a[propName]))) {
      ...
      function(done) {
        async.eachSeries(csarSpec.files, function(filePath, done) {
          ...
          fs.ensureDir(path.dirname(outputPath), function(err) {
            ...
            if (filePath !== outputPath) fs.copy(filePath, outputPath, done);
            else done();
          });
        }, done);
      },
