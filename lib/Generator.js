...
const prettify = require('prettify-xml'); // pretty-data
    ...
    async.series([
      ...
      function(done) {
        ...
        async.eachSeries(_.values(csarSpec.artifacts), function(artifact, done) {
          async.eachSeries(_.keys(artifact), function(key, done) {
            const val = artifact[key];
            done = _.once(done);

            if (_.isPlainObject(val) && val['$toscafy.fetch'] && val.to) {
              artifact[key] = val.to;

              const filePath = path.join(context.outputDir, val.to);

              fs.ensureDir(path.dirname(path.dirname(filePath)), function(err) {
                request(val['$toscafy.fetch']).on('error', (err) => {
                  done(new Error('fetch failed: ' + val['$toscafy.fetch'] + '; ' + err));
                }).pipe(fs.createWriteStream(filePath)).on('error', (err) => {
                  done(new Error('fetch failed: ' + val['$toscafy.fetch'] + '; ' + err));
                }).on('finish', done);
              });
            } else if (_.isPlainObject(val) || _.isArray(val)) {
              async.eachSeries(_.keys(val), function(innerKey, done) {
                const innerVal = val[innerKey];
                done = _.once(done);

                if (_.isPlainObject(innerVal) && innerVal['$toscafy.fetch'] && innerVal.to) {
                  val[innerKey] = innerVal.to;

                  const innerFilePath = path.join(context.outputDir, innerVal.to);

                  fs.ensureDir(path.dirname(path.dirname(innerFilePath)), function(err) {
                    request(innerVal['$toscafy.fetch']).on('error', (err) => {
                      done(new Error('fetch failed: ' + innerVal['$toscafy.fetch'] + '; ' + err));
                    }).pipe(fs.createWriteStream(innerFilePath)).on('error', (err) => {
                      done(new Error('fetch failed: ' + innerVal['$toscafy.fetch'] + '; ' + err));
                    }).on('finish', done);
                  });
                } else {
      ...
      function(done) {    
          ...
          _.forEach(csarSpec.artifacts, (a, aName) => {
            ...
            if (refProperties[a.type]) {
              _.forEach(refProperties[a.type], (propName) => {
                if (_.isString(a[propName]) &&
                   !_.includes(context.filesAbs, path.resolve(context.workingDir, a[propName])) &&
                   !_.includes(context.filesAbs, path.resolve(context.outputDir, a[propName]))) {
                  throw new Error(`file '${a[propName]}' of artifact '${aName}' does not exist or is not part of working and output directory`);
                }
              });
            }
          });
        ...
        if (context.camelize) {
          ...
          _.forEach(csarSpec.artifacts, (a, aName) => {
            ...
            if (artifacts[newName]) newName = _.upperFirst(_.camelCase(aName + '_' + shortid.generate()));
          ...
          _.forEach(csarSpec.topologies, (top, topName) => {
            ...
            if (topologies[newName]) newName = _.upperFirst(_.camelCase(topName + '_' + shortid.generate()));
          ...
          _.forEach(csarSpec.topologies, (top, topName) => {
            ...
            _.forEach(top.nodes, (n, nName) => {
              ...
              if (nodes[newName]) newName = _.upperFirst(_.camelCase(nName + '_' + shortid.generate()));
          ...
          _.forEach(csarSpec.topologies, (top, topName) => {
            _.forEach(top.relationships, (r, rName) => {
              ...
              if (relationships[newName]) newName = _.upperFirst(_.camelCase(rName + '_' + shortid.generate()));
      ...
      function(done) {
        ...
        if (_.includes(context.filesLow, context.propertiesFilename)) {
          context.propertiesFilename = 'properties-' + shortid.generate() + '.xsd';
        }

        fs.writeFile(path.resolve(context.outputDir, context.propertiesFilename), properties, done);
      },
      function(done) {
        const definitions = prettify(_.template(definitionsTpl)({
          spec: csarSpec,
          artifactTypes: knownArtifactTypes,
          relationshipTypes: knownRelationshipTypes
        }), {
          indent: 2
        });

        context.definitionsFilename = 'definitions.xml';

        if (_.includes(context.filesLow, context.definitionsFilename)) {
          context.definitionsFilename = 'definitions-' + shortid.generate() + '.xml';
        }

        fs.writeFile(path.resolve(context.outputDir, context.definitionsFilename), definitions, done);
      },
      function(done) {
