...
module.exports = () => {
  ...
  const generate = (csarspec, context, done) => {
    ...
    async.series([
      ...
      function(done) {
        if (!context.apiWrap) return done();

        async.eachSeries(_.keys(csarspec.artifacts), function(artifactName, done) {
          const artifact = csarspec.artifacts[artifactName];

          apiWrapper.wrap(artifact, {
            baseDir: context.outputDir,
            artifactName: artifactName
          }, (err, wrappedArtifact) => {
            if (err) return done(err);

            wrappedArtifact.properties = { ArtifactProperties: data2xml('ArtifactProperties', wrappedArtifact.properties) };
