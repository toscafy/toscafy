...
const runBuiltinFunctions = (collection, context, done) => {
  async.eachSeries(_.keys(collection), function(key, done) {
    ...
    if (funcName === '$toscafy.fetchAsFile') {
      ...
      const dirPath = path.join(context.workingDir, dir);
      ...
      function(done) {    
        ...
        try {
          ...
          _.forEach(csarSpec.node_types, (nt, ntName) => {
            ...
            if (_.isString(nt.deployment_artifacts)) nt.deployment_artifacts = [ nt.deployment_artifacts ];
            else if (_.isString(nt.deployment_artifact)) nt.deployment_artifacts = [ nt.deployment_artifact ];
          ...
          _.forEach(csarSpec.topologies, (top, topName) => {
            _.forEach(top.nodes, (n, nName) => {
              if (_.isString(n.deployment_artifacts)) n.deployment_artifacts = [ n.deployment_artifacts ];
              else if (_.isString(n.deployment_artifact)) n.deployment_artifacts = [ n.deployment_artifact ];
