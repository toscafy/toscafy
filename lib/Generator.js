...
const runBuiltinFunctions = (collection, context, done) => {
  async.eachSeries(_.keys(collection), function(key, done) {
    ...
    if (_.includes(['$toscafy.addFile', '$toscafy.addDir'], funcName)) {
      ...
      const sourcePath = path.resolve(context.workingDir, source);
      ...
      got(url, { encoding: null }).then(response => {
        ...
        done(new Error('fetch failed: ' + url + '; ' + err + '; response status code ' + err.statusCode));
      });
        ...
        try {
          ...
          _.forEach(csarSpec.artifacts, (a, aName) => {
            _.forEach(a.references, (ref) => {
              ...
              if (!_.includes(context.filesAbs, path.join(context.workingDir, ref)) &&
                  !_.includes(context.filesAbs, path.join(context.outputDir, ref)) &&
                  !_.includes(context.filesAbs, path.join(context.workingDir, ref) + '/') &&
                  !_.includes(context.filesAbs, path.join(context.outputDir, ref) + '/')) {
                throw new Error(`'${ref}' of artifact '${aName}' does not exist or is not part of working and output directory`);
              }
            });
          });
