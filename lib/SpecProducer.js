...
module.exports = () => {
  ...
  const produce = (input, context, done) => {
    ...
    const artifactSpecFile = path.join(os.tmpdir(), 'toscafy-artifactspec-' + shortid.generate() + '.json');
    ...
    async.series([
      ...
      function(done) {
        ...
        const ia = {
          ...
          references: [ { '$toscafy.addFile': artifactSpecFile, filename: path.join(nodeTypeName, path.basename(artifactSpecFile)) } ]
        }
        ...
        _.forEach(result.spec.parameters_schema, (prop, propName) => {
          ...
          ia.properties.mappings[propNameCamel] = {
            to: prop.mapping,
            name: propName
          }
        });
        ...
        if (result.type === 'file') {
          ...
          ia.references.push({ '$toscafy.addFile': archiveFile, filename: path.join(nodeTypeName, path.basename(archiveFile)) });
