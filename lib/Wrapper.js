...
module.exports = () => {
  ...
  const wrap = (artifact, context, done) => {
    ...
    let apispec = artifact.properties.apispec;
    ...
    //if (artifactSpecFile && !artifactFilesArchive) {
    //  return done(new Error('artifact-spec.json exists, but cannot find artifact-files archive in references'));
    //} // artifactFilesArchive is ignored if there is no artifactSpecFile
