...
module.exports = () => {
  ...
  const genpack = (csarSpec, context, done) => {
    ...
    context.outputFile = context.outputFile || path.join(os.tmpdir(), 'toscafy-csar-' + shortid.generate() + '.' + context.format);
    context.outputFile = path.resolve(context.outputFile);
    context.tempDir = path.join(os.tmpdir(), 'toscafy-temp-' + shortid.generate());
